// resources/js/Pages/Scraping/Index.jsx
    ...
    return (
        ...
        >
            ...
            <div style={{ display: "flex", gap: 20, marginBottom: 12 }}>
                <Stat
                    label="Products"
                    value={job.scraped_products.toLocaleString()}
                />
                {job.platform_detected && (
                    <Stat label="Platform" value={job.platform_detected} />
                )}
                {job.started_at && (
                    <Stat label="Started" value={job.started_at} />
                )}
                {job.completed_at && (
                    <Stat label="Finished" value={job.completed_at} />
                )}
            </div>
...
export default function ScrapingIndex({ websites, jobs: initialJobs }) {
    ...
    const [submitting, setSubmitting] = useState(false); // replaces useForm.processing
    ...
    // ✅ FIX: use router.post() directly — passes selected IDs at call time,
    // no async setData timing issue
    const handleStart = () => {
        ...
        router.post(
            "/scraping/start",
            { website_ids: selected }, // ← data passed directly here
            {
                preserveScroll: true,
                onSuccess: () => {
                    setSelected([]);
    ...
    useEffect(() => {
        const hasActive = initialJobs?.data?.some((j) =>
            ["pending", "running"].includes(j.status),
        );
        if (hasActive) {
            setPolling(true);
                ...
                >
                    ...
                    >
                        <div>
                            ...
                            >
                                {selected.length} of {websites.length} selected
                            </div>
                        </div>
                        <div style={{ display: "flex", gap: 6 }}>
                            ...
                            <button onClick={clearAll} style={smallBtn}>
                                Clear
                            </button>
                        </div>
                    </div>
                    ...
                    {allDisplayJobs.length === 0 ? (
                        ...
                        >
                            ...
                            <div style={{ fontSize: 12, marginTop: 4 }}>
                                Select websites and click "Start Scraping"
                            </div>
                        </div>
                    ) : (
                        <div
                                    ...
                                    <tbody>
                                        {historyJobs.map((j, i) => (
                                            ...
                                            >
                                                ...
                                                >
                                                    {j.scraped_products?.toLocaleString()}
                                                </td>
                                                <td
                                                    style={{
                                                        padding: "11px 16px",
